...
class MyForceFields (object):
    ...
    def __init__ (self, file_names): #pdb_file_name):
        ...
        for tree in trees:
            ...
            if element is not None:
                for at_type in element.findall('Type'):
                    ...
                    atomMass    = Decimal(at_type.attrib['mass'])
                    atomElement = None
        ...
        # k' = 1/2 k
        ene_conv = Decimal( (units.kilojoule_per_mole/(units.nanometer*units.nanometer)).conversion_factor_to( units.kilocalorie_per_mole/(units.angstrom*units.angstrom)) )
        len_conv = Decimal(units.nanometer.conversion_factor_to(units.angstrom))
        ...
        for tree in trees:
            ...
            if element is not None:
                for bond in element.findall('Bond'):
                    ...
                    bond_length = Decimal(bond.attrib['length'])*len_conv
                    bond_k      = Decimal(bond.attrib['k'])*Decimal(0.5)*ene_conv
                    harmBond    = HarmonicBondData (type1, type2, bond_length, bond_k)
                    self._harmonicBonds.append ( harmBond )
                    

        ene_conv = Decimal( units.kilojoule_per_mole.conversion_factor_to (units.kilocalorie_per_mole) )
        for tree in trees:
            ...
            if element is not None:
                for angle in element.findall('Angle'):
                    ...
                    ang_length = Decimal(angle.attrib['angle'])
                    ang_k      = Decimal(angle.attrib['k'])*Decimal(0.5)*ene_conv
            ...
            if element is not None:
                ...
                for proper in element.findall('Proper'):
                    ...
                    while 'phase%d'%index in proper.attrib:
                        ...
                        phase = Decimal(proper.attrib['phase%d'%index])
                        k     = Decimal(proper.attrib['k%d'%index])*ene_conv
                ...
                for improper in element.findall('Improper'):
                    ...
                    phase       = Decimal(improper.attrib['phase1'])
                    k           = Decimal(improper.attrib['k1'])*ene_conv
                    torsionData.periodicity.append(periodicity)
                    torsionData.phase.append(phase)
                    torsionData.k.append(k)
        ...
        ene_conv = Decimal(units.kilojoules_per_mole.conversion_factor_to(units.kilocalories_per_mole) )
        len_conv = Decimal(units.nanometers.conversion_factor_to(units.angstrom) )
        # Load NonbondedForce
        for tree in trees:
            ...
            if element is not None:
                ...
                if 'coulomb14scale' in element.attrib:
                    self._nonbonds_coulomb14scale = Decimal(element.attrib['coulomb14scale'])
                if 'lj14scale' in element.attrib:
                    self._nonbonds_lj14scale      = Decimal(element.attrib['lj14scale'])
                for atom in element.findall('Atom'):
                    typeName = atom.attrib['type']
                    
                    epsilon  = Decimal(atom.attrib['epsilon'])*ene_conv
                    sigma    = Decimal(atom.attrib['sigma'])  *len_conv
