...
def xml_ff_atom_types (data, prm, ff_prefix):
  ...
  element_list += ["K", "Ca", "Sc", "Ti", "V", "Cr", "Mn", "Fe", "Co", "Ni", "Cu", "Zn"]
...
def xml_ff_bond_force (data, prm, ff_prefix):
  ...
  forceConstConversionFactor = (kilocalorie_per_mole/(angstrom*angstrom)).conversion_factor_to(kilojoule_per_mole/(nanometer*nanometer))
...
def xml_ff_angle_force (data, prm, ff_prefix):
  ...
  forceConstConversionFactor = (kilocalorie_per_mole/(radian*radian)).conversion_factor_to(kilojoule_per_mole/(radian*radian))
...
def xml_ff_torsion_force (data, prm, ff_prefix):
  forceConstConversionFactor = (kilocalorie_per_mole).conversion_factor_to(kilojoule_per_mole)
...
def xml_ff_nonbond_force (data, prm, ff_prefix):
  ...
  for ii in range (len(prm_atom_type)):
    ...
    if not at_type in atom_type_list:
      ...
      sig = "%20.16f"%Decimal(rmin_val*rmin2sig*lengthConversionFactor)
