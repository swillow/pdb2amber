...
def xml_ff_info(data):
    """
    Add Force field information into Element (data)
...
def xml_ff_atom_types(data, prm, ff_prefix):
    """
    Add AtomType into Element (data)
    ...
    element_list += ["Na", "Mg", "Al", "Si", "P", "S", "Cl", "Ar"]
    element_list += ["K", "Ca", "Sc", "Ti", "V",
...
def xml_ff_residues(data, prm, ff_prefix):
    """
    Add Residues into Element (data)
...
def xml_ff_angle_force(data, prm, ff_prefix):
    """
    Add Angle Force into Element (data)
    ...
    forceConstConversionFactor = (kilocalorie_per_mole/(radian*radian)
                                  ).conversion_factor_to(kilojoule_per_mole/(radian*radian))
    ...
    for ii in range(0, len(raw_data), 4):
        ...
        # amber (k (x-x0)^2) ---> openmm (0.5 k' (x-x0)^2) : k' = 2k
        k0 = "%20.6f" % (float(ang_k0[ityp])*forceConstConversionFactor*2.0)
...
def xml_ff_torsion_force(data, prm, ff_prefix):
    """
    Add Torsion Force into Element (data)
    ...
    for ii in range (0, len(raw_data), 5):
        ...
        if ( int(raw_data[ii+3]) > 0): # Proper
            ...
            if not typeID in proper_typ_list:
                ...
                ff_proper.set('type1',ff_prefix+"-"+prm_atom_type[it].upper())
                ff_proper.set('type2',ff_prefix+"-"+prm_atom_type[jt].upper())
                ff_proper.set('type3',ff_prefix+"-"+prm_atom_type[kt].upper())
                ff_proper.set('type4',ff_prefix+"-"+prm_atom_type[lt].upper())
    ...
    for ii in range (0, len(raw_data), 5):
        ...
        if ( int(raw_data[ii+3]) < 0): # improper
            ...
            if not typeID in improp_typ_list:
                ...
                ff_improp.set('type1',ff_prefix+"-"+prm_atom_type[it].upper())
                ff_improp.set('type2',ff_prefix+"-"+prm_atom_type[jt].upper())
                ff_improp.set('type3',ff_prefix+"-"+prm_atom_type[kt].upper())
                ff_improp.set('type4',ff_prefix+"-"+prm_atom_type[lt].upper())
...
def xml_ff_nonbond_force (data, prm, ff_prefix):
    """
    Add Nonbond Force into Element (data)
    ...
    lengthConversionFactor = angstrom.conversion_factor_to(nanometer)
    energyConversionFactor = kilocalorie_per_mole.conversion_factor_to(kilojoule_per_mole)
    ...
    ff_nonbnd_att = ET.SubElement(ff_nonbnd_frc, 'UseAttributeFromResidue')
    ...
    for ii in range (len(prm_atom_type)):
        ...
        if not at_type in atom_type_list:
            atom_type_list.append(at_type)
            index = int(prm._raw_data['NONBONDED_PARM_INDEX'][numTypes*ityp+ityp])-1
            if index < 0: continue
            ...
            eps = "%10.7f"%(eps_val*energyConversionFactor)
            sig = "%20.16f"%(rmin_val*rmin2sig*lengthConversionFactor)
...
if __name__ == "__main__":
    ...
    opts, args = getopt.getopt (argv, "hp:x:f:", ["help=","prmtop=","xml=","ff="])

    if (len(opts) == 0):
        print ("write_xml.py -p <prmtop> -x <xml> -f <prefix_forcefield>")
        sys.exit(2)

    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print ("write_xml.py -p <prmtop> -x <xml>")
            sys.exit(1)
        elif opt in ("-p", "--prmtop"):
            prmtop_fname = arg
        elif opt in ("-x", "--xml"):
            xml_fname = arg
        elif opt in ("-f", "--ff"):
